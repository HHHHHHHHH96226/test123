export default async (request, context) => {
    ...
    let result = await fetch("https://www.85la.com/internet-access/free-network-nodes", request);
    ...
    const bgg = `https://metacubex-subconverter-1.zeabur.app/sub?target=clash&url=${encodeURI(match[1])}&insert=false&config=https%3A%2F%2Fraw.githubusercontent.com%2FACL4SSR%2FACL4SSR%2Fmaster%2FClash%2Fconfig%2FACL4SSR_Online_Full.ini&filename=85LA&emoji=true&list=false&tfo=false&scv=true&fdn=false&expand=true&sort=false&new_name=true`;
    console.log(bgg)
    return await fetch(bgg, request);;
}
